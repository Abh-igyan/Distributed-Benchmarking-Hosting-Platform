...
async def background_deploy(submission_id, zip_path):
    try:
        endpoint = await sandbox.deploy(submission_id, zip_path)
        containers_data=sandbox.containers.get(submission_id,{})
        
        sandbox.containers[submission_id] = {
            "container": containers_data.get("container"), # preserved from sandbox.deploy
            "status": "running",
            "endpoint": endpoint
        }
        ...
        async with httpx.AsyncClient() as client:
            ...
            else: raise Exception("Container health check failed")
            correctness_score = await run_correctness_check(client, endpoint)
            ...
            if response.status_code==200:
                ...
                result["score"] = calculate_score(result, correctness_score)
        ...
        # Optionally mark it as failed in your manager so the user can see via /status
        sandbox.containers[submission_id] = {"status": "failed", "error": str(e)}

@app.post("/submit")
async def submit_code(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    async with aiofiles.open(zip_path,'wb') as f:
        while chunk := await file.read(1024* 1024):
            await f.write(chunk)
    sandbox.containers[submission_id]={"status": "building"}
